...
export const RewardDetailsDrawer = ({
  ...
  const compressImage = useCallback((file: File, maxSizeMB: number = 1): Promise<string> => {
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {isLoadingReward ? (
            ...
            <div>
              ...
              />
              {errors.cost && (
                <p className="text-red-400 text-sm mt-1">{errors.cost.message}</p>
              )}
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-300 mb-2">
                Images {isCompressing && <span className="text-blue-400">(Compressing...)</span>}
              </label>
              <div
                  ...
                  {imageUrls.map((url, index) => (
                    <div key={index} className="relative group">
                      <img
                        src={url}
                        alt={`Upload ${index + 1}`}
                        className="w-full h-20 object-cover rounded-lg border border-zinc-600"
                      />
                      <button
                        type="button"
            ...
            <div className="flex space-x-2 pt-4">
              <button
                type="submit"
                disabled={isSubmitting || imageUrls.length === 0 || isCompressing}
